	...
	const onPlaceSelected = async (place: {
		...
		for (const component of placeData.addressComponents) {
			switch (component.types[0]) {
				case 'street_number':
					address.building_name_number = component.longText;
					break;
				case 'route':
				...
				case 'postal_town':
					//case 'locality':
					address.town = component.longText;
					break;
				case 'administrative_area_level_2':
					address.county = component.longText;
					break;
				case 'country':
					address.country_iso2 = component.shortText;
					break;
				case 'postal_code':
...
<section class="my-10">
	...
	<div class="grid grid-cols-1 lg:grid-cols-6 gap-x-4">
		<div class:lg:col-span-4={hasCountries} class:lg:col-span-6={!hasCountries}>
			<label class="text-sm text-gray-700" for="search">Start typing your address</label>
			<div class="relative z-50 transform rounded-xl mt-4">
				...
				{#if results.length > 0}
					<div class="absolute inset-y-0 right-0 flex py-1.5 pr-1.5">
						<kbd
							class="inline-flex items-center rounded border border-gray-200 px-1 font-sans text-xs text-gray-400 w-8 mr-1"
							>Esc</kbd
						>
						<kbd
							class="inline-flex items-center justify-center rounded border border-gray-200 px-1 font-sans text-xs text-gray-400 w-6"
							>&uArr;</kbd
						>
						<kbd
							class="inline-flex items-center rounded border border-gray-200 px-1 font-sans text-xs text-gray-400 justify-center w-6"
							>&dArr;</kbd
						>
					</div>
					<ul
		...
		<div class:lg:col-span-2={hasCountries} class:hidden={!hasCountries}>
			<label class="text-sm text-gray-700" for="search">Address country</label>
